...
int check_for_deadlock()
{
  ...
  for (i = 0; i < NUM_PHILS; i++) {
    ...
    // 13 calls to fscanf
    // 2 calls for user and system time
    for (j = 0; j < FIELDS_TO_IGNORE; j++) {
      fscanf(statf, "%*s"); // just read to skip
    }
    ...
    if ((new_user_time != user_time[i]) || (new_sys_time != sys_time[i])) {
      // no deadlock
      deadlock = 0;
      user_progress[i] = new_user_time-user_time[i]; // get new progress
      sys_progress[i] = new_sys_time-sys_time[i];
      user_time[i] = new_user_time; //update time
