...
void echoohce(char** strs) {
  ...
  for (iter = strs; *iter != NULL; ++iter)
    printf("%s ", *iter); // the pointer needs to be 
  ...
  // wrong.
  for (; iter != stop_beginning; --iter) // It should stop at point stored in stop_beginning
    printf("%s ", *iter);
...
int main(int argc, char** argv) {
  ...
  bug_info.sentence[6] = NULL; // null terminator for string
  
  // Initialize num_bugs_on_mars
  bug_info.num_bugs_on_mars = 0; // Should be initialized BEFORE print
  ...
  free(bug_info.sentence[2]); // Free memory from old string
  ...
  bug_info.sentence[2] = NULL; // memory was freed, re-set it to NULL
